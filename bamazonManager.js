...
function initialPrompt() {
  inquirer
    ...
    function viewProduct(){
        var queryStr = "SELECT item_id,product_name FROM product";
        connection.query(queryStr,function(err, data) {
              //console.log(data);
              if (err) throw err;
              //var item = data;
              for(var i=0; i<data.length;i++){
              console.log(data[i].item_id + '-'+ data[i].product_name);
              }
    }
    )};
    
    function viewLowInventory(){
        var queryStr = "SELECT item_id,product_name, stockQuantity FROM product";
        connection.query(queryStr,function(err, data) {
        console.log(data)
    }
    )};
...
function validateNumeric(value) {
    ...
    function addNewProduct(){
        ...
        ]).then(function(input) {// console.log('input: ' + JSON.stringify(input));
